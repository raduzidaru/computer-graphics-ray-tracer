...
Mesh createCubeMesh(float sideLength)
{
    ...
    // Half the side length to center the cube at the origin
    ...
    // Create vertices for each face (6 faces, 4 vertices per face)
    for (int face = 0; face < 6; ++face) {
        int startIdx = cubeMesh.vertices.size();
        for (int i = 0; i < 4; ++i) {
            Vertex vertex;
            vertex.position = positions[i + (face < 3 ? 0 : 4)];
        ...
        cubeMesh.triangles.push_back({ startIdx, startIdx + 1, startIdx + 2 });
        cubeMesh.triangles.push_back({ startIdx, startIdx + 2, startIdx + 3 });
    }
